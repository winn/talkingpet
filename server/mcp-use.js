...
const TOOL_FAMILIES = [
  ...
  {
    tool: /set_rgb|rgb/i,
    text: /สี|rgb|แดง|เขียว|น้ำเงิน|เหลือง|ม่วง|ขาว|ฟ้า|color/i,
  },
  {
    tool: /set_led|^led|หลอด/i,
    text: /ไฟ|หลอด|led|ติด|ดับ/i,
  },
  { tool: /buzzer/i, text: /buzzer|ออด|เสียง|ดัง|บีบ/i },
];
...
export function buildArguments(tool, userText) {
  const kit = kitArguments(tool, userText);
  if (kit) return kit;
...
function kitArguments(tool, userText) {
  const name = String(tool?.name || "");
  const text = String(userText || "");
  if (/get_kit_status/i.test(name)) return { args: {}, assumedPlace: false };
  if (/set_led/i.test(name)) {
    const pin = text.match(/\b(15|16|17)\b/);
    let state = "toggle";
    if (/เปิด/.test(text) || /\bon\b/i.test(text) || /ติด/.test(text)) state = "on";
    else if (/ปิด|off|ดับ/i.test(text)) state = "off";
    return { args: { pin: pin ? Number(pin[1]) : 15, state }, assumedPlace: false };
  }
  if (/buzzer/i.test(name)) {
    let state = "toggle";
    if (/เปิด/.test(text) || /\bon\b/i.test(text) || /ดัง/.test(text)) state = "on";
    else if (/ปิด|off|เงียบ/i.test(text)) state = "off";
    return { args: { state }, assumedPlace: false };
  }
  if (/set_rgb|rgb/i.test(name)) {
    if (/ปิด|off/i.test(text) && !/สี|color|แดง|เขียว|น้ำเงิน|เหลือง|ม่วง|ขาว|ฟ้า/.test(text)) {
      return { args: { state: "off" }, assumedPlace: false };
    }
    const named = Object.keys(COLORS).find((word) => text.toLowerCase().includes(word));
    if (named) return { args: { color: COLORS[named] }, assumedPlace: false };
    const hex = text.match(/#?([0-9A-Fa-f]{6})\b/);
    if (hex) return { args: { hex: hex[1] }, assumedPlace: false };
    return { args: { state: "toggle" }, assumedPlace: false };
  }
  return null;
}
